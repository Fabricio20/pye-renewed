<script>
    ...
    async function DoImport(playlist) {
        const yt_playlist = await CreatePlayList(playlist.name);
        if (yt_playlist.error) {
            errors[playlist.name] = playlist.items.length;
            return;
        }
        for (const item of playlist.items) {
            let data, time = 60000;
            do {
                data = await InsertTrack(yt_playlist.id, item);
                if (data.code === 429) {
    ...
    playlists.subscribe(async (data) => {
        // -- Display playlists
        ...
        // -- Import items
        for (const playlist of data) {
            await DoImport(playlist);
        }
        finished = true;
        console.log('Finished importing all playlists');
    });
...
<section>
    ...
    {#if waiting}
        <div class="wait">
            Encountered a rate limit while importing videos... waiting.. [Attempt #{wait_count}]
        </div>
    {/if}
    <div class="playlists">
    ...
    {#if finished === true}
        <div>
            <Button onClicked="{onFinish}">Finish</Button>
        </div>
    {/if}
</section>
